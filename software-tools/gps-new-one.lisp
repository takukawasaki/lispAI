(load "~/Dropbox/lisp-AI-book/ch-4-early-AI/gps-four.lisp")
...
(defun tree-search  (states goal-p successors combiner)
  "Find a state that satisfies goal-p. Start with states,
    and search according to successora and combiner."
...
(defun tree-search-loop (states goal-p successors combiner)
  (loop
...
(defun depth-first-search (start goal-p successors)
  "Search new states cirst until goal is reached "
  (tree-search (list start) goal-p successors #'append))

(defun binary-tree (x) (list (* 2 x) (+ 1 (* 2 x))))

(defun is-old (value) #'(lambda (x) (eql x value)))

(defun prepend (x y) "Prepend y to start of x" (append y x))

(defun breadth-first-search (start goal-p successors)
  "Search old states first until goal ids reached "
...
(defun best-first-search (start goal-p successors cost-fn)
  "Search lowest cost states first until goal is reaches"
...
(defun beam-search (start goal-p successors cost-fn beam-width)
  "Search highest scoring states first until goal is reached,
   but nrver consider more than beam-width states at a time"
...
(defparameter *cities*
  '((Atlanta         84.23 33.45) (Los Angeles        118.15 34.03)
    ...
    (Indianapolice   86.10 39.46) (Tampa               82.27 27.57)
    (Jacksonville    81.40 30.22) (Victoria           123.21 48.25)
    (Kansas-city     94.35 39.06) (Wilmington          77.57 34.14)))


(defun neighbors (city)
  "Find all cties within 1000 kilometers"
...
(defun air-distance (city1 city2)
  "the great circle distance (x,y,z coordinates of a point on a sphere."
...
(defun print-path (path &optional (stream t) depth)
  ...
  (format stream "#<Path to ~acost ~,1f>"
